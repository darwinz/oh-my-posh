...
1. **Comment on the PR**: Use the GitHub MCP `create_issue_comment` tool to post findings directly on the
   originating pull request (using the PR number identified in Phase 1).
   - Check existing PR comments first (via `list_issue_comments`) to avoid duplicate diagnostics for the same run.
